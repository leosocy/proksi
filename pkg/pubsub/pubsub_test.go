...
func TestBaseNotifier(t *testing.T) {
	...
	watcher.On("Update", mock.Anything).Return()
	// notify with empty watchers
	notifier.Notify(pxy)
	watcher.AssertNotCalled(t, "Update", pxy)
	// attach
	notifier.Attach(watcher)
	notifier.Notify(pxy)
	watcher.AssertNumberOfCalls(t, "Update", 1)
	// attach another
	anotherWatcher := new(mocks.Watcher)
	anotherWatcher.On("Update", mock.Anything).Return()
	notifier.Attach(anotherWatcher)
	notifier.Notify(pxy)
	watcher.AssertNumberOfCalls(t, "Update", 2)
	anotherWatcher.AssertNumberOfCalls(t, "Update", 1)
	// detach all
